...
class AppIdAndKeyActivity : BaseActivity() {
    companion object {
        private const val AESKEY = "U2FsdGVkX19X5NAwZN8I2oWsRPaYGJPy"
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tv_appid.text = project.appId
        tv_key.text = PRIVATE_KEY
    }
    ...
    fun coypToClipe(view: View) {
        ...
        val mClipData = ClipData.newPlainText(
            ...
            AESCrypt.encrypt(
                AESKEY,
                JSONObject().put("uid", project.appId).put("key", PRIVATE_KEY).toString()
            )
        )
        // 将ClipData内容放到系统剪贴板里。
    ...
    fun jiema(view: View) {
        val pwd = et_mw.text.toString().trim()
        et_mw.setText(AESCrypt.decrypt(AESKEY, pwd))
